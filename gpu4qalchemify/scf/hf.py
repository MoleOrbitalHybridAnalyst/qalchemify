def merged_scf_generator(SCF, molA, molB, singleA, singleB, dualA, dualB,
    ...
    from gpu4pyscf.scf import uhf, ghf
    import numpy as np
    import cupy as cp
    import types
    ...
    def generator(l):
        ...
        def get_hcore(mf, *args, **kwargs):
            ...
            vorb = cp.asarray(mf.mol.get_vorb())
            return h0 + vorb
        ...
        def fermi_occ(mu, mo_energy, sigma):
            occ = cp.zeros_like(mo_energy)
            de = (mo_energy - cp.asarray(mu)) / sigma
            occ[de<40] = 1 / (cp.exp(de[de<40])+1)
            return occ
        def get_occ(mf, mo_energy=None, mo_coeff=None):
            ...
            if is_uhf:
                ...
                def nelec_a(mu):
                    return sum(fermi_occ(mu, moe_a, mf.sigma)).get()
                def nelec_b(mu):
                    return sum(fermi_occ(mu, moe_b, mf.sigma)).get()
                from scipy.optimize import root
                sol = root(lambda mu: nelec_a(mu) - na, moe_a[max(0,int(na)-1)].get(), tol=1e-12)
                mu_a = sol['x'][0]
                sol = root(lambda mu: nelec_b(mu) - nb, moe_b[max(0,int(nb)-1)].get(), tol=1e-12)
                mu_b = sol['x'][0]
                mf.mu = cp.asarray((mu_a, mu_b))
                ...
                moe = mo_energy
                def nelec(mu):
                    return 2 * sum(fermi_occ(mu, moe, mf.sigma)).get()
                from scipy.optimize import root
                sol = root(lambda mu: nelec(mu) - na - nb, moe[max(0,int(na)-1)].get(), tol=1e-12)
                mf.mu = cp.asarray(sol['x'][0])
                mo_occ = 2 * fermi_occ(mf.mu, moe, mf.sigma)
                occ_a, occ_b = mo_occ / 2, mo_occ / 2

            f = occ_a[(occ_a > 0) & (occ_a < 1)]
            mf.entropy  = -cp.dot(f, cp.log(f)) - cp.dot(1-f, cp.log(1-f))
            f = occ_b[(occ_b > 0) & (occ_b < 1)]
            mf.entropy += -cp.dot(f, cp.log(f)) - cp.dot(1-f, cp.log(1-f))
            return mo_occ

        def init_guess_by_1e(mf, mol=None, breaksym=None):
            if is_rhf:
                return mf.__class__.init_guess_by_1e(mf, mol=mol)
            else:
                from pyscf.lib import logger
                from pyscf.scf.uhf import _break_dm_spin_symm
                if mol is None: mol = mf.mol
                if breaksym is None: breaksym = mf.init_guess_breaksym
                logger.info(mf, 'Initial guess from hcore.')
                ...
                return cp.array(dma), cp.array(dmb)
        ...
        def energy_tot_lgrad(mf):
            dm = mf.make_rdm1()
            h1_lgrad = cp.asarray(mf.mol.get_hcore_lgrad() + mf.mol.get_vorb_lgrad())
            if is_rhf:
                eg = cp.einsum('pq,qp->', h1_lgrad, dm)
            elif is_uhf:
                eg = cp.einsum('pq,xqp->', h1_lgrad, dm)
            else:
                raise NotImplementedError()
            eg += cp.asarray(mf.mol.energy_geom_res_lgrad())
            eg += cp.asarray(mf.mol.energy_nuc_lgrad())
            if do_smearing:
                if is_uhf:
                    eg += cp.dot(mf.mu, cp.asarray(mf.mol.get_nelec_lgrad()))
                else:
        ...
        mf.init_guess_by_1e = types.MethodType(init_guess_by_1e, mf)
